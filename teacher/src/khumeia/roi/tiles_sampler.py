...
class RandomPerItemSampler(RandomSampler):
    ...
    def sample_tiles_from_candidates(self, candidate_tiles):
        sampled_tiles = []
        item_keys = roi_list_utils.get_items_in_list(candidate_tiles)

        for item_key in item_keys:
            candidate_tiles_ = roi_list_utils.filter_tiles_by_item(candidate_tiles, item_key)
            if self.target_label is not None:
                candidate_tiles_ = roi_list_utils.filter_tiles_by_label(candidate_tiles_, self.target_label)
...
class StratifiedSampler(TilesSampler):
    ...
    def sample_tiles_from_candidates(self, candidate_tiles):
        sampled_tiles = []
        labels = roi_list_utils.get_labels_in_list(candidate_tiles)

        for label in labels:
            candidate_tiles_ = roi_list_utils.filter_tiles_by_label(candidate_tiles, label)
...
class StratifiedPerItemSampler(StratifiedSampler):
    ...
    def sample_tiles_from_candidates(self, candidate_tiles):
        sampled_tiles = []
        labels = roi_list_utils.get_labels_in_list(candidate_tiles)
        items = roi_list_utils.get_items_in_list(candidate_tiles)

        for label in labels:
            for item in items:
                candidate_tiles_ = roi_list_utils.filter_tiles_by_item_by_label(candidate_tiles, item, label)
...
class BackgroundToPositiveRatioSampler(TilesSampler):
    ...
    def sample_tiles_from_candidates(self, candidate_tiles):
        non_background_classes = roi_list_utils.get_labels_in_list(candidate_tiles)
...
class BackgroundToPositiveRatioPerItemSampler(BackgroundToPositiveRatioSampler):
    ...
    def sample_tiles_from_candidates(self, candidate_tiles):
        ...
        for item in items:
            candidate_tiles_ = roi_list_utils.filter_tiles_by_item(candidate_tiles, item)
            non_background_classes = roi_list_utils.get_labels_in_list(candidate_tiles_)
            non_background_classes.remove("background")

            nb_non_background_tiles = 0

            for label in non_background_classes:
                candidate_tiles__ = roi_list_utils.filter_tiles_by_label(candidate_tiles_, label)
            ...
            # Sample tiles
            candidate_tiles__ = roi_list_utils.filter_tiles_by_label(candidate_tiles_, "background")
